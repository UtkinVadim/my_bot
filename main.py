...
def buttons(call):
    ...
    if call.data == "item_two":
        bot.send_message(call.from_user.id, 'В третьих - ему очень нравится программирование и всё, что связано с миром IT!',
                         reply_markup=keyboard.keyboard_item_three)
    if call.data == "item_three":
        bot.send_message(call.from_user.id, 'В четвертых - он никогда не отчаиваится, если что-то не получается, а добивается желаемого результата!',
    ...
    if call.data == "item_six_yes":
        bot.send_message(call.from_user.id, 'Ура! Напишите ему на почту utkinvadim1991@gmail.com, или позвоните ему по телефону +7-981-687-38-20. Он будет очень рад этой новости!')
    if call.data == "item_six_no":
        bot.send_message(call.from_user.id, 'Хорошо, зато теперь вы знаете немного больше про Вадима!=) Всего Вам доброго и хорошего дня!=)')
